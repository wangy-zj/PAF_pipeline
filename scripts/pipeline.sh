...
dada_raw="dada_db -k $key_raw -b $bufsz_raw -p -w -l -c $numa"  #assign memory from NUMA node  [default: all nodes]
dada_bmf="dada_db -k $key_bmf -b $bufsz_bmf -p -w -l -c $numa"
...
process="$beamform_command -i $key_raw -o $key_bmf" # need to add other configurations as well
$echo "$process\n"
...
# Start udp2db
$udp_command -f $hdr_fname -n $nsecond_report -N $nsecond -k $key_raw -s $nblocksave
sleep 1s
