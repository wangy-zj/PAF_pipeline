...
hdr_root=$project_root/header/paf_test.header
...
save_raw_dir=/home/hero/data/data_raw
...
# 输入ringbuffer
dada_db -k $key_input -n $block_num -b $dada_dtsz -l -w&
pids+=(`echo $! `)
keys+=(`echo $key_input `)
sleep 1s 
# GPU ringbuffer
...
#output ringbuffer
dada_db -k $key_output -n $block_num -b $dada_dtsz -l -w&
pids+=(`echo $! `)
keys+=(`echo $key_output `)
sleep 1s 
$echo "created all ring buffers for the first GPU\n"
...
#将ringbuffer数据写入文件
#dada_dbdisk -k $key_input -D $save_raw_dir -o -z -v&
#pids+=(`echo $! `)
#sleep 1s
#$echo "save input ring buffer\n"
...
dada_junkdb -g -n -r 16000 -z -k $key_input -t 20 $hdr_root
pids+=(`echo $! `)
sleep 1s 
