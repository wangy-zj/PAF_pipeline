...
int main(int argc, char *argv[]){  
  ...
  std::cout << input_dbuf_size << std::endl;
  ...
  FILE *unpack_fp = fopen("../data/unpack.bin", "wb");
  FILE *fft_fp = fopen("../data/fft.bin", "wb");

  float *h_unpacked = NULL;
  cuComplex *h_ffted = NULL;
  checkCudaErrors(cudaMallocHost(&h_unpacked, nsamp_packed*sizeof(float)));
  checkCudaErrors(cudaMallocHost(&h_ffted, nsamp_fft*sizeof(cuComplex)));
  ...
  while(!ipcbuf_eod(input_dblock)){
    ...
    checkCudaErrors(cudaMemcpy(h_unpacked, d_unpacked, nsamp_packed*sizeof(float), cudaMemcpyDeviceToHost));

    if(nblock == 0){

      for(int i = 0; i < 8192; i++){
	std::cout << h_unpacked[i] << std::endl;
      }
    }
    //fwrite(h_unpacked, nsamp_packed*sizeof(float), 1, unpack_fp);
    fwrite(h_unpacked, sizeof(float), nsamp_packed, unpack_fp);
    
    checkCudaErrors(cufftExecR2C(fft_plan, d_unpacked, d_ffted));

    checkCudaErrors(cudaDeviceSynchronize());  
    checkCudaErrors(cudaMemcpy(h_ffted, d_ffted, nsamp_fft*sizeof(cuComplex), cudaMemcpyDeviceToHost));
    if(nblock == 0){
      
      for(int i = 0; i < 4097; i++){
	std::cout << h_ffted[i].x << "\t" << h_ffted[i].y << std::endl;
      }
    }
    
    //fwrite(h_ffted, nsamp_fft*sizeof(cuComplex), 1, fft_fp);
    fwrite(h_ffted, sizeof(cuComplex), nsamp_fft, fft_fp);
    //fwrite(h_ffted, sizeof(float), 2*nsamp_fft, fft_fp);
  ...
  checkCudaErrors(cudaFreeHost(h_unpacked));
  checkCudaErrors(cudaFreeHost(h_ffted));

  checkCudaErrors(cudaFree(d_packed));
  checkCudaErrors(cudaFree(d_unpacked));
  checkCudaErrors(cudaFree(d_ffted));
