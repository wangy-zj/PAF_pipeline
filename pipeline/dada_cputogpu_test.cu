#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif

#include <getopt.h>
#include <iostream>
#include <cublas_v2.h>
#include <cublas_api.h>

#include "../include/cuda/cuda_utilities.h"
#include "../include/test.h"
#include "../include/krnl.h"
#include "../include/dada_header.h"
#include "../include/dada_util.h"

#include "dada_cuda.h"
#include "beamform.hh"

void usage(){
  fprintf(stdout,
	  "process - process data from a PSRDADA ring buffer with input_key and\n"
	  "           write result to another PSRDADA ring buffer with gpu_key\n"
	  "Usage: process [options]\n"
	  " -cpu_key/-i <key>       Hexadecimal shared memory key of input PSRDADA ring buffer [default: %x]\n"
	  " -gpu_key/-g <key>      Hexadecimal shared memory key of GPU PSRDADA ring buffer [default: %x]\n"
	  " -help/-h                  Show help\n",
	  DADA_DEFAULT_BLOCK_KEY,
	  DADA_DEFAULT_BLOCK_KEY+10
	  );
}

int main(int argc, char *argv[]){  
  struct option options[] = {
			     {"cpu_key",         1, 0, 'i'},
			     {"gpu_key",           1, 0, 'g'},
			     {"help",              0, 0, 'h'}, 
			     {0, 0, 0, 0}
  };

  int input_key = DADA_DEFAULT_BLOCK_KEY;
  int gpu_key = DADA_DEFAULT_BLOCK_KEY+10;

  while (1) {
    unsigned ss;
    unsigned opt=getopt_long_only(argc, argv, "i:g:h", 
				  options, NULL);
    if (opt==EOF) break;
    
    switch (opt) {
      
    case 'i':
      key_t input_key_tmp;
      ss = sscanf(optarg, "%x", &input_key_tmp);
      if(ss != 1) {
        fprintf(stderr, "PROCESS_ERROR: Could not parse input key from %s, \n", optarg);
        fprintf(stderr, "which happens at \"%s\", line [%d], has to abort.\n",  __FILE__, __LINE__);
        exit(EXIT_FAILURE);
      }
      else{
	      input_key = input_key_tmp;
      }
      break;

    case 'g':
      key_t gpu_key_tmp;
      ss = sscanf(optarg, "%x", &gpu_key_tmp);
      if(ss != 1) {
        fprintf(stderr, "PROCESS_ERROR: CoHDUt \"%s\", line [%d], has to abort.\n",  __FILE__, __LINE__);
        exit(EXIT_FAILURE);
      }
      else{
        gpu_key = gpu_key_tmp;
      }
      break;

    case 'h':
      usage();
      exit(EXIT_SUCCESS);
      
    case '?':
    default:
      break;
    }
  }

// 绑定输入ringbuffer
  dada_hdu_t *input_hdu = dada_hdu_create(NULL);
  dada_hdu_set_key(input_hdu, input_key);
  if(dada_hdu_connect(input_hdu) < 0){ 
    fprintf(stderr, "PROCESS_ERROR:\tCan not connect to input hdu with key %x\n"
	    "which happens at \"%s\", line [%d], has to abort.\n",
	    input_key, __FILE__, __LINE__);
    exit(EXIT_FAILURE);    
  }  
  ipcbuf_t *input_dblock = (ipcbuf_t *)(input_hdu->data_block);
  ipcbuf_t *input_hblock = (ipcbuf_t *)(input_hdu->header_block);
  
  if(dada_hdu_lock_read(input_hdu) < 0) {
    fprintf(stderr, "PROCESS_ERROR:\tError locking input HDU, \n"
	    "which happens at \"%s\", line [%d], has to abort.\n",
	    __FILE__, __LINE__);
    exit(EXIT_FAILURE);
  }
  fprintf(stdout, "PROCESS_INFO:\tWe have input HDU locked\n");

// 绑定GPU ringbuffer
  dada_hdu_t *gpu_hdu = dada_hdu_create(NULL);
  dada_hdu_set_key(gpu_hdu, gpu_key);
  if(dada_hdu_connect(gpu_hdu) < 0){ 
    fprintf(stderr, "PROCESS_ERROR:\tCan not connect to output hdu with key %x\n"
	    "which happens at \"%s\", line [%d], has to abort.\n",
	    gpu_key, __FILE__, __LINE__);
    exit(EXIT_FAILURE);    
  }  
  ipcbuf_t *gpu_dblock = (ipcbuf_t *)(gpu_hdu->data_block);
  ipcbuf_t *gpu_hblock = (ipcbuf_t *)(gpu_hdu->header_block);
  
  if(dada_hdu_lock_write(gpu_hdu) < 0) {
    fprintf(stderr, "PROCESS_ERROR:\tError locking GPU HDU write, \n"
	    "which happens at \"%s\", line [%d], has to abort.\n",
	    __FILE__, __LINE__);
    exit(EXIT_FAILURE);
  }

// copy ringbuffer header
  char *input_hbuf = ipcbuf_get_next_read(input_hblock,NULL);
  if (!input_hbuf){
    fprintf(stderr, "Could not get next read header block\n");
    exit(EXIT_FAILURE);
  }

  char *gpu_hbuf = ipcbuf_get_next_write(gpu_hblock);
  if (!gpu_hbuf){
    fprintf(stderr, "Could not get next GPU write header block\n");
    exit(EXIT_FAILURE);
  }
  memcpy(gpu_hbuf, input_hbuf, DADA_DEFAULT_HEADER_SIZE);
  
  ipcbuf_mark_cleared(input_hblock);
  ipcbuf_mark_filled(gpu_hblock,DADA_DEFAULT_HEADER_SIZE);

  int block_num = 1;
  // set up CUDA
  while(!ipcbuf_eod(input_dblock)){

    //fprintf(stdout, "We are at %d block\n", nblock);
    // block memory copy,
    char *input_cbuf = ipcbuf_get_next_read(input_dblock, NULL);
    if(!input_cbuf){
      fprintf(stderr, "Could not get next read data block\n");
      exit(EXIT_FAILURE);
    }
    char *gpu_ibuf = ipcbuf_get_next_write(gpu_dblock);
    if(!gpu_ibuf){
      fprintf(stderr, "Could not get next gpu write data block\n");
      exit(EXIT_FAILURE);
    }
  
    unsigned bytes_block_input  = ipcbuf_get_bufsz(input_dblock);
    cudaStream_t stream;
    cudaStreamCreate(&stream);
    checkCudaErrors(cudaMemcpyAsync(gpu_ibuf, input_cbuf, bytes_block_input,cudaMemcpyHostToDevice,stream));
    cudaStreamSynchronize(stream);
    cudaStreamDestroy(stream);
    ipcbuf_mark_cleared(input_dblock);
    ipcbuf_mark_filled(gpu_dblock,bytes_block_input);


    if(block_num%100==0){
      fprintf(stdout, "We have copied %d blocks from CPU to GPU\n", block_num);
    }
    block_num++;
  }
  dada_hdu_unlock_read(input_hdu);
  dada_hdu_unlock_write(gpu_hdu);
  dada_hdu_destroy(input_hdu);
  dada_hdu_destroy(gpu_hdu);
    
  return EXIT_SUCCESS;
}

